...
In the antiloaded case the majority of the breakdowns are happening at the end of the structure. No evidence of migration towards the beginning of the structure is present. This can be ascribed to the limited input power~(6.5 MW) and the fields profile along the structure, that follows the gradient profile in Fig.~\ref{3grad}. As a result, the field superposition pf the incident and reflected RF is not sufficiently high to trigger any upstream breakdown, resulting in a short or absent migration.

In the loaded case, the majority of the breakdowns are migrating to the beginning of the structure. In this case most of the breakdowns are triggered in the first part of the structure, and the field has a maximum in the first cell, since the incident RF fields gets reduced along the structure by the beam-loading . These two factors might justify the behaviour in Fig.~\ref{migration_l}, as the maximum of the superimposed incident and refelcted RF should be at the beginning of the structure.
