...
%Using the set of equation (2.1) and asking that the electric field is normal to the surface in any time and anywhere to avoid power dissipation due to the Joule effect (so $\vec{E} \times \vec{n} = 0$), it can be derived the set of admitted solutions for the propagation of the EM waves. Among the solutions, the most important are the \textit{Transverse Magnetic} solutions (T.M.), where the axial component of the magnetic field is null, and the \textit{Transverse Electric} solutions (T.E.), which is the same for the electric field.
