%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%											%
%	MSc THESIS									%
%											%
%	TEST OF THE BEAM EFFECT ON A VACUUM ARC OCCURRENCE IN A HIGH-GRADIENT		%
%	ACCELERATING STRUCTURE FOR THE CLIC PROJECT					%
%											%
%											%
%	by 		Eugenio Senes 			(UniTo and CERN)   		%
%			eugenio.senes@gmail.com						%
%											%
%	Avisor: 	Prof. Martino Gagliardi 	(UniTo)				%
%	Co-advisor:	Dr. Frank Tecker 		(CERN)				%
%	Examiner:	Prof. Ferruccio Balestra	(UniTo)				%
%											%
%	based on my template in https://github.com/esenes/Unito-thesis-template		%
%											%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% DOCUMENT CLASS (alternative to book is 'report')
% Print just right page or both sides (comment the other one)
\documentclass[12pt,a4paper,openright,oneside]{book}  %%One sided

%% SET MARGINS OF THE PAGES
\usepackage{geometry}
\geometry{a4paper,portrait, left=40mm, right=30mm, top=35mm, bottom=30mm}

%% HEADERS AND FOOTERS
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhf{} %clears default header and footer
\rhead{} %right head
\lhead{ \leftmark} %left head
\rfoot{\thepage}
%%consider using also chead, cfoot, lfoot
%coherce the plain stile to this (e.g. the first page of every chapter)
\fancypagestyle{plain}{
	\fancyhf{}
	\rfoot{\thepage}
	\renewcommand{\headrulewidth}{0pt}
	\renewcommand{\footrulewidth}{0pt}
}
%% CLEAR PAGE WITHOUT NUMBER AT THE BEGINNING OF CHAPTERS
\let\origdoublepage\cleardoublepage
\newcommand{\clearemptydoublepage}{%
  \clearpage
  {\pagestyle{empty}\origdoublepage}%
}
%% ALLOW PAGE ROTATION
\usepackage{lscape}

%% HYPERTEXT SETUP
\usepackage{hyperref}
\hypersetup{
    colorlinks,
    citecolor=black,
    filecolor=black,
    linkcolor=black,
    urlcolor=black
}
%% PDF SETTINGS
\hypersetup
{
    pdfauthor={Eugenio Senes},
    pdftitle={MSc-Thesis},
    pdfsubject={},
    pdfkeywords={vacuum-arcs,RF,breakdown}
}
%% FONTS AND SYMBOLS
\usepackage[utf8]{inputenc}	%%input font setting
\usepackage[T1]{fontenc} 		%%font for automatic recognition of letters with the accent
\usepackage{amsfonts}		%%fonts for the mathematical rendering of formulas
\usepackage{amssymb}
\usepackage{amsmath}
\usepackage{ mathrsfs }
%% CHAPTERS STRUCTURE
\usepackage[italian,english]{babel} %%Set English as main language of the document
%% FIGURES
\usepackage{graphicx}
\usepackage{subfigure}	%%Allow side by side figure
%% TABLES
\usepackage{multirow}
\usepackage{booktabs}
%%CAPTIONS
\usepackage{caption}
%% BIBLIOGRAPHY
\usepackage[babel]{csquotes}


%% HYPENATON
\hyphenation{ ac-ce-le-ra-zio-ne ac-ce-le-ra-to-re pres-ent-ly plas-ma}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% BEGIN DOCUMENT
\begin{document}

%%%% HEAD  OF THE DOCUMENT
\frontmatter
%%FRONT PAGE
\input{head/frontPage.tex}
\clearemptydoublepage
%%DEDICATION (the initial quote)
\input{head/dedication}
\clearemptydoublepage
%%ABSTRACT
\input{head/abstract}
\input{head/abstractITA}
\clearemptydoublepage
%%INDEXES
%summary
\tableofcontents
\clearemptydoublepage
%%%% BODY OF THE DOCUMENT
\mainmatter
%%INTRODUCTION
\input{body/introduction.tex}
\clearemptydoublepage
%% CHAPTERS
% add any further chapter file here
\input{body/chapter1.tex}
\clearemptydoublepage
\input{body/chapter2.tex}
\clearemptydoublepage
\input{body/chapter3_1.tex}
\clearemptydoublepage
\input{body/chapter3.tex}
\clearemptydoublepage
\input{body/chapter4.tex}
\clearemptydoublepage
\input{body/chapter5.tex}
\clearemptydoublepage


%%%% TAIL OF THE DOCUMENT
\backmatter
\input{tail/acknowledgments}
\addcontentsline{toc}{chapter}{Acknowledgments}
%list of abbreviations
\clearemptydoublepage
\input{tail/abbreviations.tex}
\addcontentsline{toc}{chapter}{List of Abbreviations}
\clearemptydoublepage
%list of figures
\addcontentsline{toc}{chapter}{\listfigurename}
\listoffigures
\clearemptydoublepage
%list of tables
\addcontentsline{toc}{chapter}{\listtablename}
\listoftables
\clearemptydoublepage
%bibliography
\addcontentsline{toc}{chapter}{Bibliography}
\bibliography{bibliography/bibThesis}
\bibliographystyle{ieeetr}
\clearemptydoublepage


\end{document}
